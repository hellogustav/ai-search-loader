...
type GlowAnimationV1_1Props = {
  ...
  className?: string;
  hideShape?: boolean; // Hide the @ symbol shape
  reverse?: boolean; // Run animation in reverse direction
};

const GlowAnimationV1_1: React.FC<GlowAnimationV1_1Props> = ({
  duration = 2.8, // 30% faster (4 * 0.7)
  trailCoverage = 0.784, // Reduced by 30% from 1.12 - 78.4% coverage
  speedFactor = 1,
  className = '',
  hideShape = false,
  reverse = false
  ...
  useEffect(() => {
    ...
    // Setup canvas for overlay - larger to accommodate glow and shimmers
    canvas.width = 160; // 120px + 20px padding each side
    canvas.height = 160;
    ...
    const pathPoints = new Array(samples).fill(0).map((_, i) => {
      ...
      // Map different sections of the path to appropriate colors with smooth wraparound
      if (progress < 0.22) {
        ...
        // Smooth transition from blue back to orange (wraparound)
        const t = (progress - 0.9) / 0.1;
        color = {
          r: Math.round(101 * (1 - t) + 255 * t), // 65BDEB to FF9C26
          g: Math.round(189 * (1 - t) + 156 * t),
          b: Math.round(235 * (1 - t) + 38 * t)
        };
      }

      // Add offset to center in larger canvas (canvas is 160px, SVG is 120px)
    ...
    const trailLength = Math.min(Math.floor(samples * trailCoverage), samples * 2); // Allow up to 200% but cap for performance
    let currentIndex = 0;
    ...
    const animate = (timestamp: number) => {
      ...
      // Build trail with wraparound support for >100% coverage
      trailSegments.length = 0;
      for (let i = 0; i < trailLength; i++) {
        // For trails longer than the path, wrap around multiple times
        const wrappedIndex = i % samples;
        // If reverse, build trail in opposite direction
        const idx = reverse
          ? (currentIndex + wrappedIndex) % samples  // Trail ahead of current position
          : (currentIndex - wrappedIndex + samples) % samples; // Trail behind current position
        const pt = pathPoints[idx];
        // Adjust intensity decay for longer trails - smoother fade
        const intensity = Math.pow(1 - (i / trailLength), 1.8); // Less aggressive decay for smoother fade
        trailSegments.push({ x: pt.x, y: pt.y, intensity, color: pt.color });
      }

      // Create shimmers periodically near the head (only when shape is visible)
      if (!hideShape && timestamp - lastShimmerTime > 100) { // Create shimmer every 100ms
        const shimmerCount = Math.random() < 0.3 ? 2 : 1; // Sometimes create 2
        for (let i = 0; i < shimmerCount; i++) {
          const offsetX = (Math.random() - 0.5) * 18; // Slightly larger offset for more space
          const offsetY = (Math.random() - 0.5) * 18;
          shimmers.push({
            x: headPoint.x + offsetX,
            y: headPoint.y + offsetY,
            age: 0,
            maxAge: 500, // 500ms lifetime
            size: Math.random() * 1.5 + 0.5, // Small size
      ...
      // Update and clean up old shimmers
      for (let i = shimmers.length - 1; i >= 0; i--) {
        shimmers[i].age += 16; // Assuming ~60fps
        shimmers[i].rotation += 0.15; // Rotate
        if (shimmers[i].age >= shimmers[i].maxAge) {
          shimmers.splice(i, 1); // Remove old shimmers
        }
      }

      // REMOVED additive blending to prevent white buildup - use normal blending
      ctx.globalCompositeOperation = 'source-over';

      // Optimized single-pass rendering with adaptive quality

      // REMOVED atmospheric glow completely - it was creating too much white
      // We'll rely on the trail itself and shadow effects for the glow appearance

      // Draw trail - adaptive segment drawing for performance
      ctx.save();
      ctx.lineCap = 'round';
      ctx.lineJoin = 'round';

      // Increase glow when shape is hidden
      const glowMultiplier = hideShape ? 3.2 : 1;

      // Adaptive segment drawing - draw fewer segments for fainter parts
      for (let i = 1; i < trailSegments.length; i++) {
        const seg = trailSegments[i];

        // Skip very faint segments
        if (seg.intensity < 0.005) continue;

        // Adaptive sampling based on intensity
        let skip = false;
        if (seg.intensity < 0.2 && i % 3 !== 0) {
          skip = true; // Draw every 3rd segment for very faint trail
        } else if (seg.intensity < 0.5 && i % 2 !== 0) {
          skip = true; // Draw every 2nd segment for medium trail
        }
        // Draw every segment for intensity >= 0.5 (bright head area)

        if (skip) continue;

        // Find the previous segment to connect from (go back to find last drawn segment)
        let prevIndex = i - 1;
        ...
        // Set individual segment style with tapering near head
        // Create narrower head by tapering the trail width
        let widthMultiplier = 1;
        if (seg.intensity > 0.8) {
          // Taper the trail near the head (intensity > 0.8)
          widthMultiplier = 0.5 + (0.5 * (1 - seg.intensity) / 0.2); // Tapers from 0.5 to 1.0
        }
        ...
        // Apply graduated shadow blur for more distributed glow
        if (seg.intensity > 0.2) {
          // Much gentler shadow blur that extends along more of the trail
        ...
        // Draw this segment - extend slightly for smooth connections
        ctx.beginPath();
        ctx.moveTo(prevSeg.x, prevSeg.y);

        // Extend the line slightly past the current point for overlap
      ...
      // Third pass: REMOVED white spots to show more color
      // White spots were causing too much white buildup
      // Keeping this commented in case we want to add back very subtle ones later
      /*
      ctx.save();
      for (let i = 0; i < trailSegments.length; i += 5) {
        const seg = trailSegments[i];
        if (seg.intensity < 0.7) continue;
        ctx.fillStyle = `rgba(255, 255, 255, ${0.15 * seg.intensity})`;
        ctx.beginPath();
        ctx.arc(seg.x, seg.y, 0.3 * seg.intensity, 0, Math.PI * 2);
        ctx.fill();
      }
      ctx.restore();
      */

      // Get head color for use in shimmers and head
      const headColor = headPoint.color;

      // Draw shimmers/sparkles with colors matching the head (only when shape is visible)
      if (!hideShape) {
        ctx.save();
        shimmers.forEach(shimmer => {
          const ageRatio = shimmer.age / shimmer.maxAge;
          const opacity = 1 - ageRatio; // Fade out
          const scale = 1 + ageRatio * 0.5; // Grow slightly

          ctx.save();
          ctx.translate(shimmer.x, shimmer.y);
          ctx.rotate(shimmer.rotation);

          // Draw colored shimmer matching the gradient
          ctx.fillStyle = `rgba(${headColor.r}, ${headColor.g}, ${headColor.b}, ${opacity * 0.6})`;
          ctx.shadowBlur = 3 * opacity;
          ctx.shadowColor = `rgba(${headColor.r}, ${headColor.g}, ${headColor.b}, 0.8)`;

          // Draw cross shape for sparkle effect
          ctx.fillRect(-shimmer.size * scale * 0.3, -shimmer.size * scale, shimmer.size * scale * 0.6, shimmer.size * scale * 2);
          ctx.fillRect(-shimmer.size * scale, -shimmer.size * scale * 0.3, shimmer.size * scale * 2, shimmer.size * scale * 0.6);
      ...
      // Draw head - pure saturated color only
      ctx.save();

      // Increase head glow when shape is hidden (but more subtle)
      const headGlowMultiplier = hideShape ? 1.4 : 1;

      // Use pure gradient color without any white or brightening
      ctx.shadowBlur = 2.5 * headGlowMultiplier; // Further reduced glow
      ctx.shadowColor = `rgba(${headColor.r}, ${headColor.g}, ${headColor.b}, 0.9)`;

      // Draw main head dot with full saturation - slightly smaller
      ctx.fillStyle = `rgba(${headColor.r}, ${headColor.g}, ${headColor.b}, 1)`;
      ctx.beginPath();
      ctx.arc(headPoint.x, headPoint.y, 1.2 * (hideShape ? 1.4 : 1), 0, Math.PI * 2); // Reduced from 2 to 1.2
      ctx.fill();

      // Add second layer for intensity without white
      ctx.shadowBlur = 2 * headGlowMultiplier;
      ctx.fillStyle = `rgba(${headColor.r}, ${headColor.g}, ${headColor.b}, 0.8)`;
      ctx.beginPath();
      ctx.arc(headPoint.x, headPoint.y, 0.6 * (hideShape ? 1.4 : 1), 0, Math.PI * 2); // Reduced from 1 to 0.6
      ctx.fill();
  ...
  }, [duration, trailCoverage, speedFactor, hideShape, reverse]);

  return (
    <div className={`glow-animation-v1-1-container ${className}`} ref={containerRef}>
      {/* Original @ Symbol Shape - conditionally rendered */}
      <svg className="base-svg-v1-1" width="120" height="120" viewBox="0 0 162 162" fill="none" style={{ opacity: hideShape ? 0 : 1 }}>
        <path
