...
/**
 * GlowAnimationV8 - Neon trail animation with AI search loading text
 *
 * Extends V7 with animated gradient text suitable for candidate search applications.
 * Features multiple text transition effects and customizable messages.
 *
 * @component
 */
...
type TextAnimation = 'fade' | 'slide' | 'typewriter' | 'flip' | 'blur';

type GlowAnimationV8Props = {
  duration?: number; // Animation duration in seconds (0.5-10s)
  reverse?: boolean; // Run animation in reverse direction
  ariaLabel?: string; // Accessibility label
  showLoadingText?: boolean; // Show loading text below spinner
  textChangeInterval?: number; // How often to change text in seconds
  textAnimation?: TextAnimation; // Text transition animation type
  customMessages?: string[]; // Custom loading messages
};

// Default loading messages for AI candidate search
const defaultMessages = [
  "Analyzing candidate profiles...",
  "Matching skills to requirements...",
  "Scanning experience levels...",
  "Evaluating cultural fit...",
  "Checking availability...",
  "Reviewing portfolio highlights...",
...
const GlowAnimationV8: React.FC<GlowAnimationV8Props> = ({
  duration = 2.24,
  reverse = false,
  ariaLabel = "AI search loading indicator",
  showLoadingText = true,
  textChangeInterval = 2.5,
  textAnimation = 'fade',
  customMessages
}) => {
  ...
  const messages = useMemo(() =>
    customMessages && customMessages.length > 0 ? customMessages : defaultMessages,
    [customMessages]
  );
  ...
  const validatedTextInterval = useMemo(() => {
    const t = Number(textChangeInterval);
    if (isNaN(t) || t < 0.5) return 0.5;
    if (t > 10) return 10;
    return t;
  }, [textChangeInterval]);
  ...
  useEffect(() => {
    ...
    textIntervalRef.current = setInterval(changeText, validatedTextInterval * 1000);
  ...
  }, [showLoadingText, messages, validatedTextInterval, textAnimation]);

  // Handle typewriter effect
  useEffect(() => {
    if (textAnimation !== 'typewriter' || !showLoadingText) return;

    const currentMessage = messages[currentMessageIndex];
    if (typewriterIndex < currentMessage.length) {
      const timeout = setTimeout(() => {
        setDisplayedText(currentMessage.substring(0, typewriterIndex + 1));
        setTypewriterIndex(prev => prev + 1);
      }, 50); // Typing speed

      return () => clearTimeout(timeout);
    }
  }, [typewriterIndex, currentMessageIndex, messages, textAnimation, showLoadingText]);

  // Update displayed text for non-typewriter animations
  useEffect(() => {
    if (textAnimation !== 'typewriter' && showLoadingText) {
      setDisplayedText(messages[currentMessageIndex]);
    }
  }, [currentMessageIndex, messages, textAnimation, showLoadingText]);

  // Canvas animation logic (same as V7)
  useEffect(() => {
    ...
    if (!window.requestAnimationFrame) {
      console.warn('GlowAnimationV8: requestAnimationFrame not supported');
      return;
    }
    ...
    if (!ctx) {
      console.warn('GlowAnimationV8: Canvas 2D context not supported');
      return;
    }
    ...
    try {
      ...
      const glowMultiplier = 2.0;
      const headGlowMultiplier = 1.2;
      ...
      const animate = (timestamp: number) => {
        ...
        // If reduced motion is preferred, show static trail
      ...
      console.error('GlowAnimationV8: Error during initialization or animation', error);
  ...
  const getTextAnimationClass = () => {
    ...
    const baseClass = `loading-text-v8 loading-text-${textAnimation}`;
    return isTextAnimating ? `${baseClass} text-animating` : baseClass;
  };

  return (
    <div
      className="glow-animation-v8-container"
      role="img"
    ...
    >
      ...
      <canvas ref={canvasRef} className="glow-animation-v8-canvas" />
...
export default GlowAnimationV8;